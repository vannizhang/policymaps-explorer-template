...
const addToMyFavGroup = (itemId = ''): Promise<ShareOperationResponse> => {
    ...
    return new Promise(async (resolve, reject) => {
        if (!favGroupId || !token) {
            ...
            reject({
                message:
                    'favorite group id and token are required to add fav item, please sign in first',
                isSignInRequired: true,
            });
        } else {
            try {
                const toggleShareAsMyFavRes = await toggleShareAsMyFav(
                    itemId,
                    true
                );
...
const removeFromMyFavGroup = (itemId = ''): Promise<ShareOperationResponse> => {
    ...
    return new Promise(async (resolve, reject) => {
        if (!favGroupId || !token) {
            ...
            reject({
                message:
                    'favorite group id and token are required to remove fav item, please sign in first',
                isSignInRequired: true,
            });
        } else {
            try {
...
const addUnSavedItemToMyFav = (): Promise<string> => {
    const itemId = localStorage.getItem(LOCAL_STORAGE_KEY_ADD)
        ? localStorage.getItem(LOCAL_STORAGE_KEY_ADD)
        : null;
...
const toggleShareAsMyFav = (
    itemId: string,
    isAddingToMyFav: boolean
...
export const toggleAsMyFavItem = (itemId: string): Promise<string[]> => {
    ...
    return new Promise(async (resolve, reject) => {
        try {
            if (isInMyFavItems) {
                const removeFromMyFavGroupRes = await removeFromMyFavGroup(
                    itemId
                );
