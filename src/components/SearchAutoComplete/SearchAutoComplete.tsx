...
import { getSearchSuggest, SuggestResult } from './arcgis-online-suggest-api';
...
const SearchAutoComplete: React.FC<Props> = ({
    ...
    const searchInputOnKeyDown = (
        ...
        if (event.key === 'ArrowDown') {
            const newCandidateIndex =
                suggestionCandidateIndex + 1 < suggestions.length
    ...
    const getLabelForSuggestionItem = (title: string) => {
        ...
        const valInItemTitle = title.substring(
            idxOfVal,
            idxOfVal + searchTerm.length
        );
    ...
    const getSuggestionList = (): JSX.Element => {
        ...
        const suggestionListItems = suggestions.map((d, i) => {
            ...
            const isActiveCandidate =
                suggestionCandidateIndex === i ? 'is-active-condidate' : '';
